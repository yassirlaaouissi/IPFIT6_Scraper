...
def scrape(scrape_website=True, skip_duplicate_addresses=False):
    timeofnow = datetime.now().strftime(f"%Y-%m-%d")

    # Created driver and wait
    driver = webdriver.Chrome("gmaps_scraper/chromedriver.exe")
    ...
    workbook = xlsxwriter.Workbook(f'Output/Google Maps/GoogleMaps_{timeofnow}.xlsx')
